	...
	return (
		<>
			<div
				...
				className={cl.header_burger + ' ' + active}>
				<span></span>
			</div>
			<nav className={cl.navBar + ' ' + className}>
				{/*  Костыль)
                     По факту должен быть логотип */}
				<div className={cl.helperForCenter}></div>

				{/* Главная навигация сайта */}
				<div
					onClick={() => (active ? handleClickActive() : null)}
				...
				<div className={cl.signIn}>
					{currentUser ? (
						<Link to="/profilePage" className={cl.profileLink}>
							<div
								className={cl.profileWrapper}
								style={{
									borderColor: isSubscribe || subscribe ? '#1D79D2' : 'grey',
									boxShadow:
										isSubscribe || subscribe ? '0 0 7px #1D79D2' : '0 0 0',
								}}>
