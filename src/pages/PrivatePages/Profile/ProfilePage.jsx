...
const ProfilePage = () => {
	...
	useEffect(() => {
		get(ref(database, `Users/${auth.currentUser.uid}/isSubscribe`)).then(
			snap => {
				setIsSubscribe(snap.val());
			},
		);
		get(ref(database, `Users/${auth.currentUser.uid}/Name`)).then(snap => {
			setName(snap.val());
		});
		get(ref(database, `Users/${auth.currentUser.uid}/Surname`)).then(snap => {
			setSurname(snap.val());
		});
		get(ref(database, `Users/${auth.currentUser.uid}/Email`)).then(snap => {
			setEmail(snap.val());
		});
	}, []);
	...
	return (
		<div className={cl.wrapper}>
			<div
				...
				style={{
					borderColor: isSubscribe || isSub ? '#1D79D2' : 'grey',
					boxShadow: isSubscribe || isSub ? '0 0 7px #1D79D2' : '0 0 0',
				}}>
				<div className={cl.userInfo}>
					<div className={cl.profileImgWrapper}>
						<div
							className={cl.imgBorder}
							style={{
								borderColor: isSubscribe || isSub ? '#1D79D2' : 'grey',
								boxShadow: isSubscribe || isSub ? '0 0 7px #1D79D2' : '0 0 0',
							}}>
					...
					<div className={cl.infoWrapper}>
						...
						<div className={cl.info}>
								...
								<div className={cl.inputWrapper}>
									{surname ? (
										...
										<>
											...
											/>
											<button
												onClick={setSurnameInDB}
												className={cl.doneButton}>
												<svg
													xmlns="http://www.w3.org/2000/svg"
													className={cl.done}
													height="24"
							...
							<div>
								<label>
									Email: <span>{email ? email : null}</span>
								</label>
							</div>
							<div>
								<label>
									...
									<span
										style={{
											color: isSubscribe || isSub ? '#1D79D2' : 'white',
										}}>
										{isSubscribe || isSub ? 'Активна' : 'Не активна'}
									</span>
								</label>
							</div>
						</div>
				...
				<div className={cl.logOut}>
					<button className={cl.cancelSubButton} onClick={cancelSub}>
						Отменить подписку
					</button>
					<button className={cl.logOutButton} onClick={logOut}>
						Выйти из аккаунта
					</button>
				</div>
			</div>
