...
import SeparateCategoryButton from '../../shared/ui/Buttons/SeparateCategoryButton/SeparateCategoryButton';
...
const StringMovies = ({ category, limit }) => {
	...
	useEffect(() => {
		...
		get(category === 'Рекомендуем' ? recCat : queryCat).then(
			snap => {
				if (snap.exists) {
	...
	return (
		<div className={cl.stringMoviesWrapper}>
			<SeparateCategoryButton
				category={category}
				className={category === 'Рекомендуем' ? cl.rec : null}
			/>
			<div
				className={cl.listMovies}
				style={{
					display: fetching
						? 'flex'
						: category === 'Рекомендуем'
							? null
							: 'flex',
					flexWrap: category === 'Рекомендуем' ? 'nowrap' : 'wrap',
				}}>
				{fetching ? (
					renderSkeleton()
				) : category === 'Рекомендуем' ? (
